...
ddns.prototype.recordExists = function() {
  return request.getListOfDomainRecords(this.domain)
    .then((response) => {
      const findObj = {name: this.record, type: this.type};
...
ddns.prototype.createDomainRecord = function() {
  return this.recordExists(this.ip, this.type, this.domain)
    .then((exists) => {
      if (exists) {
        return Object.assign(exists, {
          exists: 1
        });
      } else {
        return request.createDomainRecord(this.domain, this.type, this.record, this.ip)
          .then((createdResult) => {
            return Object.assign({exists: 0}, createdResult.domain_record);
          }).catch((e) => {
            console.log(e);
          });
      }
...
ddns.prototype.updateDomainRecord = function(recordId) {
  return request.updateDomainRecord(this.domain, recordId, this.type, this.record, this.ip);
};
